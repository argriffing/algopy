...
#         assert_allclose(x, numpy.arange(N*M*K).reshape((N,M,K)))
#         assert_allclose(V, numpy.arange(P*N*M*K,D*P*N*M*K).reshape((D-1,P,N,M,K)).transpose((2,3,4,1,0)))
...
#         u = UTPM( numpy.arange(D*P*N*M*K).reshape((D,P,N,M,K)))
#         Vbar = utpm2dirs(u)
#         assert_allclose(Vbar, numpy.arange(D*P*N*M*K).reshape((D,P,N,M,K)).transpose((2,3,4,1,0)) )
